...
const one = integer => {
  if (typeof integer !== "number") return "Not a number";
  else {
...
const two = outerArray => {
  for (let i = 0; i < outerArray.length; i++) {
    outerArray[i].map(element => console.log(element));
  }
};
...
const four = city => {
  if (city.includes("Los")) {
    return city;
  } else if (city.includes("New")) {
    return city;
  } else {
...
const six = (integers, callback) => {
  ...
  for (let i = 0; i < integers.length; i++) {
    ...
    newArr.push(callback(integers[i]));
  }
  return newArr;
};
